...
const openAiConfigured = computed(() => Boolean(botSettings.value?.openai_configured))
...
const applyBotSettings = (data) => {
  ...
  botForm.value = {
    ...
    whatsapp_ai_enabled: settings.whatsapp_ai_enabled !== false,
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="pm-page-subtitle">
              Conecte o número comercial para enviar status de pedido aos clientes automaticamente.
            </p>
          </div>
        </div>
      ...
      <section v-else-if="isBlocked" class="relative overflow-hidden rounded-3xl border border-slate-800 bg-slate-950 p-8 text-white shadow-xl">
        <div class="relative z-10 max-w-2xl">
          ...
          <p class="mt-4 text-sm font-semibold leading-relaxed text-slate-300">
            ...
            Conecte com QR Code e envie status de pedido pelo WhatsApp.
          </p>
      ...
      <template v-else>
        ...
        <template v-else>
          <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
            <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              <div class="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5">
                <div>
                  <h2 class="text-lg font-black text-slate-900">Conexão</h2>
                  <p class="mt-1 text-sm font-semibold text-slate-500">
                    Instância exclusiva da loja na Evolution API.
                  </p>
                </div>
            ...
            <aside class="space-y-4">
              ...
              >
                ...
                <div class="mt-4 flex min-h-[240px] items-center justify-center rounded-2xl bg-slate-50 p-3">
                  <img
                    v-if="qrImageSrc"
                    :src="qrImageSrc"
              ...
              <div
                ...
                class="rounded-3xl border border-emerald-100 bg-emerald-50 p-6 text-center shadow-sm"
              >
                <CheckCircle class="mx-auto text-emerald-600" size="44" />
                ...
                <p class="mt-1 text-xs font-semibold leading-relaxed text-emerald-700">
                  Status de pedido enviados automaticamente aos clientes.
                </p>
              </div>
              ...
              >
                <MessageCircle class="mx-auto text-slate-300" size="44" />
          ...
          <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
            <div class="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                <h2 class="text-lg font-black text-slate-900">Mensagens automáticas</h2>
                <p class="mt-1 max-w-2xl text-sm font-semibold text-slate-500">
                  ...
                  <code
                    ...
                    :key="token"
                    class="mx-0.5 rounded bg-slate-100 px-1.5 py-0.5 text-xs font-bold text-slate-700"
                  >{{ token }}</code>
                  como variáveis.
                </p>
              </div>
          ...
          >
            <div class="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                <h2 class="text-lg font-black text-slate-900">Bot de atendimento</h2>
                <p class="mt-1 max-w-2xl text-sm font-semibold text-slate-500">
                  Responde automaticamente cardápio, horário e status do pedido. Premium inclui IA para perguntas em texto livre.
                </p>
              </div>
            ...
            <div v-if="botLoading" class="py-10 flex justify-center">
              <Loader2 class="animate-spin text-emerald-600" size="28" />
            </div>
            ...
            <div v-else class="mt-5 space-y-5">
              <label class="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <span class="text-sm font-bold leading-relaxed text-slate-600">
                  Ativar bot automático (menu com cardápio, horário e pedido)
                </span>
              </label>
              ...
              <div v-if="hasAiFeature" class="space-y-3 rounded-2xl border border-violet-100 bg-violet-50/50 p-4">
                <label class="flex items-start gap-3">
                  ...
                  <span class="text-sm font-bold leading-relaxed text-slate-700">
                    ...
                    IA para responder dúvidas (Premium)
                  </span>
                </label>

                <p v-if="!openAiConfigured" class="text-xs font-semibold text-amber-700">
                  OpenAI não configurada no servidor (`OPENAI_API_KEY`). A IA ficará desativada até configurar.
                </p>

                <div>
                  <label class="text-xs font-black uppercase tracking-wider text-slate-500">FAQ para a IA</label>
                  <textarea
                    v-model="botForm.whatsapp_ai_faq"
                    rows="4"
                    class="pm-input mt-2 w-full resize-y text-sm"
                    :readonly="!canConfigure"
                    placeholder="Ex: Não temos opção vegana. Estacionamento gratuito na rua de trás."
                  />
                </div>
              </div>
